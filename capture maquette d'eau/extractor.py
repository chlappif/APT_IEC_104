...
# TO DO : ADAPT THE CODE FOR THE LEFT SIDE OF THE SYSTEM
...
def data_extraction(ipsource, ipdest, sourcefile, destfile):
    ...
    for trame in json_data:
        ...
        if trame["_source"]["layers"]["ip"]["ip.src"] == "192.168.20.11" and trame["_source"]["layers"]["104asdu"][
            "104asdu.typeid"] == "11" and gaga == True and string != "0":
            if "IOA: 1" in trame["_source"]["layers"]["104asdu"]:
                string = string + str(demand1[-1]) + "," + str(values1[-1]) + "," + \
...
def data_extraction_lstm(ipsource, ipdest, sourcefile, destfile):
    ...
    for trame in json_data:
        ...
        if trame["_source"]["layers"]["ip"]["ip.src"] == ipsource:
            ...
            string = trame["_source"]["layers"]["frame"]["frame.time_epoch"] + "," + apci["104apci.apdulen"] + "," + \
                     apci[
                         "104apci.type"]
            string = string + "," + asdu["104asdu.typeid"] + "," + asdu["104asdu.sq"] + "," + asdu[
                "104asdu.numix"] + "," + asdu["104asdu.causetx"] + "," + asdu["104asdu.nega"] + "," + asdu[
...
# test :
data_extraction_lstm("192.168.110.10", "192.168.110.11", "./Capture Data/raspid_grosse.json",
                     "Capture Data/right_lstm_raspi_data_grosse.txt")
